...
sed "s/IMAGES/$(escape "${IMAGES}")/g" ${BR2_EXTERNAL}/board/meerkat-l4t/kernel_fdt.its.in > /tmp/kernel_fdt.its.in
for DT in ${DTS}; do
	echo "Building uImage with FDT ${DT} in ${IMAGES}"
	sed "s/DEVICETREE/$(escape "${DT}")/g" /tmp/kernel_fdt.its.in > /tmp/kernel_fdt.its
